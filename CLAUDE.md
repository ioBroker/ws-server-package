...
`@iobroker/ws-server` is a **library**, not a runnable adapter. It exports server-side classes that ioBroker WEB adapters (e.g. `ioBroker.ws`, `web`, `admin`) embed to expose ioBroker's object/state API to browser clients over WebSockets. There is no runtime entry point (no `main.ts`); `src/index.ts` only re-exports the public API.
...
The repo's own package is named `@iobroker/ws-server`, and it **also depends on a different package with the same name** (`@iobroker/ws-server`, a lower version pinned in `dependencies`). That dependency is the low-level pure-WebSocket transport.

Therefore any `import ... from '@iobroker/ws-server'` inside `src/` resolves to the **dependency**, not to this repo's code. From it the code pulls:
...
Do not assume same-name imports are self-references.
...
3. **This package** — thin glue between the two:
   ...
   - `src/lib/types.d.ts` — `WsConfig`, the adapter's `native` config shape (port, auth, secure, TLS certs, ttl, etc.).
...
`io-package.json` describes the consuming `ws` adapter (admin UI config, default `native`, instance objects) — useful as the contract for `WsConfig`, but this repo does not run as that adapter.

## Commands

```bash
npm run build      # tsc -p tsconfig.build.json, then `node tasks` (tasks.mts)
...
`node tasks` (`tasks.mts`) runs after `tsc`: it copies the bundled `@iobroker/ws` browser client into `build/` and copies the type declarations. Build output goes to `build/` (gitignored).
